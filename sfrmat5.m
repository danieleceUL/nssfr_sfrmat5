...
%     mask   = Masked region for e-SFR estimation - OvZ 2023
%      
% Returns: 
...
% Adaptions for NS-SFRs by Oliver van Zwanenberg (2023)
%******************************************************************
...
rflag = 0;
[a, mask, nlin, npix, rflag] = rotatev2(a, mask);  %based on data values
loc = zeros(ncol, nlin);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%sfr50 = freqval(2);  % frequency where SFR = 50% DJ - this is not ideal
%need to return both MTF10 and MTF50
ns = length(summary);
summary{ns+1} = e;
